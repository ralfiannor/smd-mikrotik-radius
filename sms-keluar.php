<?php
    ...
    if(isset($_GET['data']))
    {
      ...
      $query = "select * from outbox ORDER BY SendingDateTime DESC";       
      $crud->smsKeluar($query);
      exit();
    }
...
if (isset($_GET['open'])) {
    $id = $_GET['open'];
    extract($crud->getID("outbox",$id));
?>
  <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="exampleModalLabel">Pesan Keluar ke <b><?= $DestinationNumber ?></b></h4>
      </div>
...
window.actionEvents = {
    'click .lihat': function (e, value, row, index) {
      ...
      var url = 'sms-keluar.php?open='+row['id'];
    ...
    'click .hapus': function (e, value, row, index) {
      var url = 'sms-keluar.php?hapus='+row['id'];
      $.get(url, function (data) {
        var modal = $('<div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"><div class="modal-dialog" role="document"><div class="modal-content">' + data + '</div></div></div>').modal();
